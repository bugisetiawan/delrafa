...
$(document).ready(function(){
    ...
    if($('#customer_code').val() == "")
    {
        $.getJSON("transaction/Sales/get_customer", (data) => {
            ...
            if (data.status.code == 200) {
                $.each(data.response, function(i) {
                    option += '<option value="' + data.response[i].code + '" class="kt-font-dark" data-id="'+data.response[i].id+'">' +data.response[i].code+ ' | ' +data.response[i].name + ' | ' +data.response[i].address + '</option>';
                });
            }
            else
            {
        ...
        $("#customer_code").on("change", function(){
            $('#customer_id').val($(this).children("option:selected").data('id'));
        });
    }                
    ...
    $("#transaction_table").on("focus", ".transaction_input", function(){
        var transaction = new Bloodhound({
            ...
            prefetch:"finance/Payment/get_transaction/2/"+$('#customer_code').val(),
            remote:{
                url:"finance/Payment/get_transaction/2/"+$('#customer_code').val()+"/%QUERY%/",
                wildcard:"%QUERY%"
            }
        });        
        if($(this).data('autocomple_ready') != 'ok' ){            
            ...
            },
            {
                ...
                templates: {
                    ...
                    suggestion: Handlebars.compile('<div>{{date}}<br>{{invoice}}<br>TOTAL: {{grandtotal}}<br>TAGIHAN: {{account_payable}}</div>')
                }
            });               
    ...
    $("#transaction_table").on("keyup", ".transaction_disc_rp", function(){
        var transaction_disc_rp = Math.round(new Date().getTime() +(Math.random() * 100)); $(this).attr('id', transaction_disc_rp);
        var transaction_account_payable = $(this).attr('id')+1;
        var transaction_pay = $(this).attr('id')+2;
        $(this).closest('tr').find('.transaction_account_payable').attr('id', transaction_account_payable);
        $(this).closest('tr').find('.transaction_pay').attr('id', transaction_pay);
        var subtotal = Number($('#'+transaction_account_payable).val().replace(/\,/g, ""))-Number($('#'+transaction_disc_rp).val().replace(/\,/g, ""));    
        if(subtotal >= 0)
        {
            $('#'+transaction_pay).val(format_amount(String(subtotal.toFixed(2))));
        }        
        else
        {
            $('#'+transaction_disc_rp).val(0).select();
            $('#'+transaction_pay).val($('#'+transaction_account_payable).val());
            toastr.error('Diskon (Rp) tidak bisa melebihi tagihan');
        }
        calculate();
    });

    $("#transaction_table").on("keyup", ".transaction_pay", function() {
        var transaction_pay = Math.round(new Date().getTime() +(Math.random() * 100)); $(this).attr('id', transaction_pay);
        var transaction_account_payable = $(this).attr('id')+1;
        var transaction_disc_rp = $(this).attr('id')+2;        
        $(this).closest('tr').find('.transaction_account_payable').attr('id', transaction_account_payable);
        $(this).closest('tr').find('.transaction_disc_rp').attr('id', transaction_disc_rp);
        var subtotal = Number($('#'+transaction_account_payable).val().replace(/\,/g, ""))-Number($('#'+transaction_disc_rp).val().replace(/\,/g, ""));    
        if(Number($('#'+transaction_pay).val().replace(/\,/g, "")) > subtotal)
        {
            $('#'+transaction_pay).val(format_amount(String(subtotal.toFixed(2))));
            toastr.error('Pembayaran tidak bisa melebihi tagihan');
        }            
        calculate();
    });
    ...
    $('#payment_table').on('change', '.payment_method',function(){
        ...
        else if($(this).val() == 4)
        {
            if($('#customer_id').val() != "")
            {
                $.getJSON("finance/Cash_ledger/get_cash_ledger_account/4/"+$('#customer_id').val(), (data) => {
                    if(data.status.code == 200) {
                        $.each(data.response, function(i) {
                            option += '<option value="' + data.response[i].id + '" class="kt-font-dark">' +data.response[i].code+ ' | ' +data.response[i].name + '</option>';
                        });
            ...
            else
            {
                ...
                toastr.error('Mohon maaf, harap pilih pelanggan terlebih dahulu');
            }
        }
        else
        {
    ...
    $('#add_payment').on('click', function() {
        ...
        html += '<td><input type="text" class="form-control text-center date cheque_open_date" name="cheque_open_date[]" readonly></td>';
        html += '<td><input type="text" class="form-control text-center date cheque_close_date" name="cheque_close_date[]" readonly></td>';
...
function calculate()
{
    ...
    if(total_transaction_pay > 0 && total_payment_pay > 0)
    {
        ...
        else
        {
            ...
            var message = "MOHON PERHATIAN! TOTAL TRANSAKSI DAN PEMBAYARAN TIDAK SAMA";
