$(document).ready(function(){
    ...
    $("#delete_cash_ledger_in_out_btn").on('click', function(){		
		$.ajax({
			url: "Auth/verify_user_password",
			type: "POST",
			dataType: "JSON",
			data:{
				'module_url' : 'cash_ledger/in_out',
				'action' : 'delete'
			},
			success: (data) => {
				if(data.status.code == 200) 
				{
					swal.fire({
						...
						if(result.value) {
							$.ajax({
								url		: "finance/Cash_ledger/delete_cash_ledger_in_out",
								method	: "POST",
								dataType: "JSON",
								data: {
								...
								success		: (data) => {						
									if(data.status.code	== 200) 
									{							
										window.location.replace("cash_ledger/cash");
									}
									else 
									{
