...
$(document).ready(function(){
	...
	$('#from_date, #to_date, #customer_code, #sales_code, #payment_status').change(function(){		
		total_sales_receivable_report();
		table.ajax.reload();
	});

	total_sales_receivable_report();

	$('#print_sales_receivable_report').click(function(){		
		$('#filter_form').attr('target', '_blank').attr('action', 'report/Finance_report/print_sales_receivable_report').submit();
	});

	$('#print_sales_billing_report').click(function(){		
		$('#filter_form').attr('target', '_blank').attr('action', 'report/Finance_report/print_sales_billing_report').submit();
	});

	$('#print_customer_global_sales_receivable_report').click(function(){		
		$('#filter_form').attr('target', '_blank').attr('action', 'report/Finance_report/print_customer_global_sales_receivable_report').submit();
	});
	...
	var table = $("#datatable").DataTable({
		...
		ajax: {
			"url"		: "report/Finance_report/sales_receivable/", 
			"type"		: "POST",			
            "data":function(data){
        ...
        columnDefs: [
			...
			{ 
                ...
                render : function(val){
					...
					{
						return "<div class='kt-font-bold text-warning'>BELUM LUNAS</div>";
					}
					else
					{
...
function total_sales_receivable_report()
{
	$.ajax({
		type: "POST",
		url: "report/Finance_report/total_sales_receivable_report/",		
		dataType: "JSON",
		data: {
