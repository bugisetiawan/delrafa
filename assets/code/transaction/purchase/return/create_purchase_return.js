$(document).ready(function(){
		...
		if($(this).val()!="")
		{
            ...
            if($('#method').val() == 2)
            {
                $.getJSON("transaction/Purchase/get_invoice_return/"+$('#supplier_code').val(), (data) => {
                    ...
                    if (data.status.code == 200) {
                        $.each(data.response, function(i) {
                            option += '<option value="' + data.response[i].id + '" class="kt-font-dark">'+data.response[i].code+ ' | '+data.response[i].invoice+ ' | '+String(format_number(data.response[i].account_payable))+'</option>';
                        });
                    } 
                    else 
                    {
    ...
    $("#product_table").on("focus", ".product_input", function(){
        ...
        var product = new Bloodhound({
            ...
            remote:{
                url:"transaction/Purchase/get_product_return/%QUERY%/"+supplier_code+"/"+ppn,
                wildcard:"%QUERY%"
            }
        });  
