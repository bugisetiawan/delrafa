$(document).ready(function() {    	  
    ...
    $.getJSON("transaction/Sales/get_employee", (data) => {
        ...
        var value = ($('#checker_code_update').val() != null) ? $('#checker_code_update').val() : null;
        $('#checker_code').html(option).val(value).select2();
    });
...
{
    var res = 0;
    $(':input[required], option[required]', '#update_form' ).each(function(){        
        if(this.value.trim() == ''){
            res++;
        }                
    });
