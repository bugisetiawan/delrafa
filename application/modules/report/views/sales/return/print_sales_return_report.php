        ...
        <table style="width: 100%;" celpadding="1" cellspacing="1">
            <thead>
                <tr>
                    ...
                    <th class="text-center" style="border-bottom: 1px solid black;">Pelanggan</th>
                    <th class="text-center" style="border-bottom: 1px solid black;">Total</th>                    
                </tr>
            </thead>
            <tbody>
                <?php $no=1; $total_return = 0; foreach($data AS $sales_return): ?>
                <tr>
                    <td class="text-right"><?php echo $no.'.'; ?></td>
                    <td class="text-center"><?php echo date('d-m-Y', strtotime($sales_return['date'])); ?></td>
                    <td class="text-center"><?php echo $sales_return['code']; ?></td>
                    <td><?php echo $sales_return['name_c']; ?></td>
                    <td class="text-right"><?php echo number_format($sales_return['total_return'], 2, '.', ','); ?></td>
                    <?php $total_return = $total_return + $sales_return['total_return']; ?>
                </tr>
                <?php $no++; endforeach; ?>
                <tr>
